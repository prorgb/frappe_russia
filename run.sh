#!/usr/bin/env bash
# Запуск всех контейнеров. Первичное создание сайта Frappe: INIT=1 ./run.sh
...
[[ -f "$ENV_FILE" ]] || { echo "Нет $ENV_FILE — сначала sudo ./install.sh"; exit 1; }
...
if [[ "${INIT:-}" != "1" ]] && ! compose exec -T backend test -f "sites/${FRAPPE_SITE_NAME}/site_config.json" 2>/dev/null; then
  echo "Подсказка: если сайт Frappe ещё не создан, один раз выполните: INIT=1 $ROOT/run.sh"
fi
