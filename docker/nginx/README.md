...
| [`conf.d/maxon24.online-http.conf`](conf.d/maxon24.online-http.conf) | **Порт 80**: `/.well-known/acme-challenge/` (Certbot webroot) и режим для остальных запросов — см. `conf.d/includes/maxon24.online-http-mode.inc`. |
...
| [`conf.d/includes/maxon24.online-app.inc`](conf.d/includes/maxon24.online-app.inc) | Общие `proxy_*`, таймауты и все `location` (ERPNext, API-префиксы, factoring). |
| [`conf.d/includes/maxon24.online-http-mode-proxy.inc`](conf.d/includes/maxon24.online-http-mode-proxy.inc) | На 80 включить приложение по HTTP (проксирование, как для 443 без шифрования на edge). |
| [`conf.d/includes/maxon24.online-http-mode-redirect.inc`](conf.d/includes/maxon24.online-http-mode-redirect.inc) | На 80 у всех клиентских путей — редирект `301` на HTTPS (`https://$host$request_uri`). |
| [`conf.d/includes/maxon24.online-http-mode.inc`](conf.d/includes/maxon24.online-http-mode.inc) | Переключатель: активна либо ветка **proxy**, либо **redirect** (комментируйте вторую строку и раскомментируйте нужную пару строк). |

В `docker-compose.yml` для edge монтируется только каталог `./nginx/conf.d` целиком (в том числе подкаталог `includes/`). Отдельного тома для сниппетов не требуется.
...
Отредактируйте [`conf.d/includes/maxon24.online-http-mode.inc`](conf.d/includes/maxon24.online-http-mode.inc).
...
- **С TLS:** добавьте `maxon24.online-https.conf`, получите сертификаты (пути см. ниже), в `conf.d/includes/maxon24.online-http-mode.inc` включите режим **redirect** вместо **proxy**.
...
| Файл | Действие |
|------|----------|
| [`../../reload.sh`](../../reload.sh) | Рестарт **всех** контейнеров в статусе `running`; `edge-nginx` перезапускается **последним**, чтобы заново резолвить upstream (`frontend`, `marketplace`, …) после их рестарта и поднять конфиг из `nginx/conf.d` (включая `conf.d/includes/`). |
...
## Что подхватывает nginx автоматически

В образе задаётся `include /etc/nginx/conf.d/*.conf` — подключаются **только файлы вида `*.conf` непосредственно в `conf.d/`**, без подкаталогов. Файлы в `conf.d/includes/*.inc` образ **не** читает сам по себе: их подтягивает только директива `include` из ваших `.conf`.

Если после `git pull` появился каталог `conf.d/includes/` и падение вида «No such file» для `.inc`: перезапустите stack из каталога `docker/` (`compose` должен пробрасывать актуальный `./nginx/conf.d`) или выполните `docker compose up -d edge-nginx`.
